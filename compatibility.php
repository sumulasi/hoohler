	...
	private function compare_versions() {
		...
		if ( version_compare( PHP_VERSION, $this->php_min, '<' ) ) {
			...
			$this->notices[] = sprintf(
				...
				__( "<strong>%1\$s</strong> requires at least <code>PHP %2\$s</code>.\nPlease contact your hosting provider to upgrade your PHP version.", 'hooker' ),
				$this->plugin,
				$this->php_min
			);
		...
		if ( ! file_exists( dirname( __FILE__ ) . '/build' ) ) {
			...
			$this->notices[] = sprintf(
				...
				__( "%1\$s plugin requires files to be built.\nRun <code>npm install</code> to install dependencies\n<code>npm run build</code> to build the files or <code>npm run dev</code> to build the files and watch for changes.", 'hooker' ),
				$this->plugin,
				$this->php_min
			);
